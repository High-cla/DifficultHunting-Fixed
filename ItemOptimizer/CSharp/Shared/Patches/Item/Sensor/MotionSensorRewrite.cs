...
namespace ItemOptimizerMod.Patches
{
    ...
    static class MotionSensorRewrite
    {
        ...
        // ── Structs for grouping detection parameters ──

        private readonly struct DetectionFilter
        {
            public readonly bool TrigHumans;
            public readonly bool TrigPets;
            public readonly bool TrigMonsters;
            public readonly HashSet<Identifier> TargetChars;

            public DetectionFilter(bool trigHumans, bool trigPets, bool trigMonsters, HashSet<Identifier> targetChars)
            {
                TrigHumans = trigHumans;
                TrigPets = trigPets;
                TrigMonsters = trigMonsters;
                TargetChars = targetChars;
            }
        }

        private readonly struct DetectionBounds
        {
            public readonly float RangeX;
            public readonly float RangeY;
            public readonly Rectangle DetectRect;
            public readonly float MinVelSq;

            public DetectionBounds(float rangeX, float rangeY, Rectangle detectRect, float minVelSq)
            {
                RangeX = rangeX;
                RangeY = rangeY;
        ...
        private static readonly Connection[] CachedStateOut = new Connection[MaxItemId];

        // ── Frame counter for throttle (flat array, no CWT) ──
        private static readonly int[] FrameCounters = new int[MaxItemId];

        // ── NativeRuntime bypass: sensors managed by NativeRuntime skip Prefix entirely ──
        internal static readonly bool[] IsNativeManaged = new bool[MaxItemId];
        ...
        private static readonly SensorSpatialCache[] _spatialCache = new SensorSpatialCache[MaxItemId];
        ...
        internal static Connection GetStateOutConnection(MotionSensor ms)
        {
            ...
            for (int _idx = 0; _idx < connections.Count; _idx++)
            {
                var c = connections[_idx];
                if (c.Name == "state_out")
                {
                    CachedStateOut[id] = c;
                    return c;
        ...
        private static bool CheckCandidates(
            ...
            Vector2 detectPos, DetectionBounds bounds, DetectionFilter filter)
        {
            for (int j = 0; j < candidates.Count; j++)
            {
                ...
                if (Math.Abs(character.WorldPosition.X - detectPos.X) > bounds.RangeX ||
                    Math.Abs(character.WorldPosition.Y - detectPos.Y) > bounds.RangeY)
                    continue;
                ...
                if (character.IsPet)
                {
                    if (!filter.TrigPets) continue;
                }
                else if (character.IsHuman || CharacterParams.CompareGroup(character.Group, CharacterPrefab.HumanGroup))
                {
                    if (!filter.TrigHumans) continue;
                }
                else
                {
                    if (!filter.TrigMonsters) continue;
                }

                // Target character filter
                if (filter.TargetChars.Count > 0)
                {
                    bool matchFound = false;
                    foreach (Identifier target in filter.TargetChars)
                    {
                ...
                foreach (Limb limb in character.AnimController.Limbs)
                {
                    ...
                    if (limb.LinearVelocity.LengthSquared() < bounds.MinVelSq) continue;
                    if (MathUtils.CircleIntersectsRectangle(limb.WorldPosition,
                        ConvertUnits.ToDisplayUnits(limb.body.GetMaxExtent()), bounds.DetectRect))
                    {
                        return true;
                    }
                }
            ...
            Vector2 detectPos, DetectionBounds bounds, DetectionFilter filter)
        {
            for (int j = 0; j < candidates.Count; j++)
            {
                ...
                if (dx > bounds.RangeX || dy > bounds.RangeY) continue;
                ...
                if (character.AnimController?.Collider != null)
                {
                    ...
                    if (vel.LengthSquared() >= bounds.MinVelSq)
                        return true;
                }
            }
            return false;
        ...
        internal static void Execute(MotionSensor __instance, float deltaTime)
        {
            ...
            if (item.CurrentHull == null && item.Submarine != null && __instance.Target.HasFlag(MotionSensor.TargetType.Wall))
            {
                     ...
                     Math.Abs(item.Submarine.Velocity.Y) > __instance.MinimumVelocity))
                {
                    ...
                    for (int _idx = 0; _idx < cells.Count; _idx++)
                    {
                        var cell = cells[_idx];
                        ...
                        for (int _idx2 = 0; _idx2 < cell.Edges.Count; _idx2++)
                        {
                            var edge = cell.Edges[_idx2];
                ...
                for (int _idx = 0; _idx < Submarine.Loaded.Count; _idx++)
                {
                    Submarine sub = Submarine.Loaded[_idx];
                    ...
                    {
                        for (int _idx2 = 0; _idx2 < Structure.WallList.Count; _idx2++)
                        {
                            Structure wall = Structure.WallList[_idx2];
                ...
                && (UpdateAllTakeover.Enabled || NativeRuntimeBridge.IsEnabled))
            {
                ...
                for (int i = 0; i < coveredIds.Length; i++)
                {
                    ...
                    if (candidates.Count > 0 && CheckCandidates(candidates, __instance,
                        detectPos,
                        new DetectionBounds(broadRangeX, broadRangeY, detectRect, minVelSq),
                        new DetectionFilter(trigHumans, trigPets, trigMonsters, targetChars)))
                    {
                        __instance.MotionDetected = true;
                        return;
                    }
            ...
            else
            {
                ...
                for (int _idx = 0; _idx < Character.CharacterList.Count; _idx++)
                {
                    Character character = Character.CharacterList[_idx];
                    if (Math.Abs(character.WorldPosition.X - detectPos.X) > broadRangeX ||
                        Math.Abs(character.WorldPosition.Y - detectPos.Y) > broadRangeY)
                        continue;
                ...
                && (UpdateAllTakeover.Enabled || NativeRuntimeBridge.IsEnabled))
            {
                ...
                for (int i = 0; i < coveredIds.Length; i++)
                {
                    ...
                    if (candidates.Count > 0 && QuickCheckCandidates(candidates, sensor,
                        detectPos,
                        new DetectionBounds(rxExpanded, ryExpanded, default, minVelSq), // DetectRect unused in QuickCheckCandidates
                        new DetectionFilter(trigHumans, trigPets, trigMonsters, targetChars)))
                    {
                        return true;
                    }
