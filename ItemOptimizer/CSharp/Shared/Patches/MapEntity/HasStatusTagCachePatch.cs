...
namespace ItemOptimizerMod.Patches
{
    ...
    static class HasStatusTagCachePatch
    {
        ...
        internal static void PreBuildAll()
        {
            ...
            for (int _idx = 0; _idx < StatusEffect.DurationList.Count; _idx++)
            {
                var durationEffect = StatusEffect.DurationList[_idx];
                var effectTags = Ref_statusEffectTags(durationEffect.Parent);
                if (effectTags == null || effectTags.Count == 0) continue;
                for (int _idx2 = 0; _idx2 < durationEffect.Targets.Count; _idx2++)
                {
                    var target = durationEffect.Targets[_idx2];
            ...
            for (int _idx = 0; _idx < DelayedEffect.DelayList.Count; _idx++)
            {
                var delayedEffect = DelayedEffect.DelayList[_idx];
                var effectTags = Ref_statusEffectTags(delayedEffect.Parent);
                if (effectTags == null || effectTags.Count == 0) continue;
                for (int _idx2 = 0; _idx2 < delayedEffect.Targets.Count; _idx2++)
                {
                    var target = delayedEffect.Targets[_idx2];
                    if (!_cache.TryGetValue(target, out var tags))
                    {
                        tags = RentFromPool();
                        _cache[target] = tags;
