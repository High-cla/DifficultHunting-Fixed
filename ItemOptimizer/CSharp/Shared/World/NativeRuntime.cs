...
namespace ItemOptimizerMod.World
{
    ...
    public class NativeRuntime
    {
        ...
        // ── Parallel.For state (thread-static for zero-allocation dispatch) ──

        [ThreadStatic] private static NativeRuntime _parallelRuntime;
        [ThreadStatic] private static float _parallelDeltaTime;

        private static readonly Action<int> TickZoneParallelAction = i =>
        {
            var self = _parallelRuntime;
            var zone = self.Graph.Zones[i];
            if (zone.Tier >= ZoneTier.Dormant) return;
            self.TickZone(zone, _parallelDeltaTime);
        };
        ...
        public void Tick(float deltaTime, Camera cam)
        {
            ...
            {
                var c = Character.CharacterList[_idx];
        ...
        private void RunTickParallel(float deltaTime)
        {
            ...
            // Uses cached static Action<int> + [ThreadStatic] fields to avoid closure allocations
            _parallelRuntime = this;
            _parallelDeltaTime = deltaTime;
            try
            {
                Parallel.For(0, Graph.Zones.Count, TickZoneParallelAction);
            }
            finally
            {
                _parallelRuntime = null;
            }
        }
