...
namespace ItemOptimizerMod
{
    ...
    static class SpikeDetector
    {
        ...
        /// <summary>Per-thread item start timestamp. Serial access assumed — SpikeDetector runs on the main thread within the diagnostics update loop, never accessed concurrently.</summary>
        [ThreadStatic]
        ...
        private static void LogSpike(double totalMs, int gcGen0, int gcGen1, int gcGen2)
        {
            ...
            catch (Exception e) { DebugConsole.ThrowError($"[ItemOptimizer] Spike log write failed: {e.Message}"); }
        }
        ...
        {
            sb.Append(PerfProfiler.CsvEscape(ts)).Append(',');
            sb.Append(totalMs.ToString("F1")).Append(',');
            sb.Append(type).Append(',');
            sb.Append(PerfProfiler.CsvEscape(key)).Append(',');
            sb.AppendLine(PerfProfiler.CsvEscape(value));
        }
