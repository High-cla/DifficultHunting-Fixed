...
namespace ItemOptimizerMod.SignalGraph
{
    ...
    static class SignalGraphEvaluator
    {
        ...
        /// <summary>Cooldown frames before recompiling after MarkDirty(). At 60fps, 30 frames ≈ 0.5s.
        /// This batches rapid wire edits into a single recompile instead of recompiling per-change.</summary>
        private const int RecompileCooldownFrames = 30;

        /// <summary>How often (in frames) to check if accelerated items still exist.
        /// Every 60 frames ≈ 1s at 60fps. Higher = less overhead but slower dead-item cleanup.</summary>
        private const int ConditionCheckInterval = 60;

        /// <summary>Smoothing factor for AvgTickMs EMA. Lower = smoother but slower to react.</summary>
        ...
        private static readonly bool[] _accelerated = new bool[MaxItemId];
        ...
        internal static void MarkDirty()
        {
            ...
            if (!_dirty)
            {
                ...
                IsCompiled = false;
                Array.Clear(_accelerated, 0, MaxItemId);
                _dirty = true;
            }
        }
        ...
        internal static EmitDiagResult GetEmitDiagnostics(int filterTargetId = -1)
        {
            ...
            for (int i = 0; i < emits.Length; i++)
            {
                ...
                bool isAccel = targetId >= 0 && targetId < MaxItemId && _accelerated[targetId];
