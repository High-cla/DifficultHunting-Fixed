...
namespace ItemOptimizerMod.SignalGraph
{
    ...
    static class SignalGraphBuilder
    {
        ...
        {
            var candidates = DiscoverCandidates(mode);
            if (candidates.Count == 0) return null;
            LogCandidateDiagnostics(candidates);

            #region Step 2: Allocate registers for output connections
            int nextReg = 0;
            ...
            foreach (var (id, (item, comp, type)) in candidates)
            {
                ...
                foreach (var conn in item.Connections)
                {
                    ...
                    {
                        if (string.Equals(conn.Name, name, StringComparison.Ordinal)) { isGraphOutput = true; break; }
                    }
            ...
            #region Step 3: Wire resolution — resolve inputs to register indices
            ...
            #region Step 4: Topological sort (Kahn's algorithm)
            ...
            #region Step 5: Cycle detection — remaining nodes form cycles
            ...
            #region Step 6: Build wire visual map (CLIENT only, but allocated always)
            ...
            #region Step 6b: Build emit edges (accel output → non-accel downstream)
            ...
            foreach (var (id, (item, comp, type)) in candidates)
            {
                ...
                foreach (var outName in outputNames)
                {
                    ...
                    foreach (var conn in item.Connections)
                    {
                        if (conn.IsOutput && string.Equals(conn.Name, outName, StringComparison.Ordinal)) { outConn = conn; break; }
                    }
            ...
            #region Step 7: Assemble CompiledGraph
        ...
        private static Dictionary<ushort, (Item item, ItemComponent comp, SignalNodeType type)> DiscoverCandidates(int mode)
        {
            // ────────────────────────────────────
        ...
        private static void LogCandidateDiagnostics(Dictionary<ushort, (Item item, ItemComponent comp, SignalNodeType type)> candidates)
        {
            ...
            Dictionary<int, Connection> regSourceConn)
        {
            ...
            for (int i = 0; i < inputNames.Length; i++)
            {
                ...
                foreach (var conn in item.Connections)
                {
                    if (!conn.IsOutput && string.Equals(conn.Name, inputNames[i], StringComparison.Ordinal))
                    {
                        inputConn = conn;
                        break;
                    }
