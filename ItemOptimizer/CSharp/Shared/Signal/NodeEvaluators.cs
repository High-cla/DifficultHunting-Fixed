...
namespace ItemOptimizerMod.SignalGraph
{
    ...
    {
        private const float MinTimerStep = 0.1f;
            ...
            switch (node.Type)
            {
                ...
                case SignalNodeType.BoolOp_Or:
                    if (node.Component is BooleanOperatorComponent boc)
                    {
                        ...
                        _bool_tsr0[si] = Math.Max(boc.TimeFrame * 2f, MinTimerStep);
                        _bool_tsr1[si] = Math.Max(boc.TimeFrame * 2f, MinTimerStep);
                    }
                    break;
                ...
                case SignalNodeType.Compare_Equals:
                    if (node.Component is EqualsComponent ec)
                    {
                        ...
                        _eq_tsr0[si] = Math.Max(ec.TimeFrame * 2f, MinTimerStep);
                        _eq_tsr1[si] = Math.Max(ec.TimeFrame * 2f, MinTimerStep);
                    }
                    break;
                ...
                case SignalNodeType.Compare_Greater:
                    if (node.Component is GreaterComponent gc)
                    {
                        ...
                        _gt_tsr0[si] = Math.Max(gc.TimeFrame * 2f, MinTimerStep);
                        _gt_tsr1[si] = Math.Max(gc.TimeFrame * 2f, MinTimerStep);
                    }
                    break;
                ...
                case SignalNodeType.Arith_Div:
                    if (node.Component is ArithmeticComponent ac)
                    {
                        ...
                        _arith_tsr0[si] = Math.Max(ac.TimeFrame * 2f, MinTimerStep);
                        _arith_tsr1[si] = Math.Max(ac.TimeFrame * 2f, MinTimerStep);
                    }
                    break;
        ...
        public static void Evaluate(ref SignalNode node, string[] regs, float dt)
        {
            ...
            {
                case SignalNodeType.BoolOp_And:
                    EvalBoolOp(new SignalEvalContext(node.StateIndex, node.InputRegs, node.OutputRegs, regs, dt), requiredInputs: 2);
                    break;
                case SignalNodeType.BoolOp_Or:
                    EvalBoolOp(new SignalEvalContext(node.StateIndex, node.InputRegs, node.OutputRegs, regs, dt), requiredInputs: 1);
                    break;
                case SignalNodeType.Logic_Not:
                ...
                case SignalNodeType.Compare_SignalCheck:
                    ...
                    break;
                case SignalNodeType.Compare_Equals:
                    EvalEquals(new SignalEvalContext(node.StateIndex, node.InputRegs, node.OutputRegs, regs, dt));
                    break;
                case SignalNodeType.Compare_Greater:
                    EvalGreater(new SignalEvalContext(node.StateIndex, node.InputRegs, node.OutputRegs, regs, dt));
                    break;
                case SignalNodeType.Arith_Add:
                    EvalArith(node.Type, new SignalEvalContext(node.StateIndex, node.InputRegs, node.OutputRegs, regs, dt));
                    break;
                case SignalNodeType.Arith_Sub:
                    EvalArith(node.Type, new SignalEvalContext(node.StateIndex, node.InputRegs, node.OutputRegs, regs, dt));
                    break;
        ...
        private readonly struct SignalEvalContext
        {
            public readonly int Si;
            public readonly int[] InRegs;
            public readonly int[] OutRegs;
            public readonly string[] Regs;
            public readonly float Dt;

            public SignalEvalContext(int si, int[] inRegs, int[] outRegs, string[] regs, float dt)
            {
                Si = si;
                InRegs = inRegs;
                OutRegs = outRegs;
        ...
        private static void EvalBoolOp(in SignalEvalContext ctx, int requiredInputs)
        {
            string in0 = ctx.InRegs[0] >= 0 ? ctx.Regs[ctx.InRegs[0]] : null;
            string in1 = ctx.InRegs[1] >= 0 ? ctx.Regs[ctx.InRegs[1]] : null;

            // set_output: dynamically overrides the Output property (vanilla ReceiveSignal behavior)
            if (ctx.InRegs.Length > 2 && ctx.InRegs[2] >= 0)
            {
                string setOut = ctx.Regs[ctx.InRegs[2]];
                if (setOut != null)
                    _bool_output[ctx.Si] = setOut;
            }

            if (in0 != null && in0 != "0")
                _bool_tsr0[ctx.Si] = 0f;
            else
                _bool_tsr0[ctx.Si] += ctx.Dt;

            if (in1 != null && in1 != "0")
                _bool_tsr1[ctx.Si] = 0f;
            else
                _bool_tsr1[ctx.Si] += ctx.Dt;

            float tf = _bool_timeFrame[ctx.Si];
            int received = (_bool_tsr0[ctx.Si] <= tf ? 1 : 0) + (_bool_tsr1[ctx.Si] <= tf ? 1 : 0);

            string output = received >= requiredInputs ? _bool_output[ctx.Si] : _bool_falseOutput[ctx.Si];
            if (ctx.OutRegs[0] >= 0)
                ctx.Regs[ctx.OutRegs[0]] = string.IsNullOrEmpty(output) ? null : output;
        }
        ...
        private static void EvalNot(int si, int[] inRegs, int[] outRegs, string[] regs)
        {
            ...
            if (input != null)
            {
                ...
                if (outRegs[0] >= 0)
                    regs[outRegs[0]] = (input.Length == 1 && input[0] == '0' || input.Length == 0) ? "1" : "0";
            }
        ...
        private static void EvalEquals(in SignalEvalContext ctx)
        {
            string in0 = ctx.InRegs[0] >= 0 ? ctx.Regs[ctx.InRegs[0]] : null;
            string in1 = ctx.InRegs[1] >= 0 ? ctx.Regs[ctx.InRegs[1]] : null;
            ...
            if (in0 != null) { _eq_recv0[ctx.Si] = in0; _eq_tsr0[ctx.Si] = 0f; }
            else _eq_tsr0[ctx.Si] += ctx.Dt;

            if (in1 != null) { _eq_recv1[ctx.Si] = in1; _eq_tsr1[ctx.Si] = 0f; }
            else _eq_tsr1[ctx.Si] += ctx.Dt;

            float tf = _eq_timeFrame[ctx.Si];
            int received = (_eq_tsr0[ctx.Si] <= tf ? 1 : 0) + (_eq_tsr1[ctx.Si] <= tf ? 1 : 0);
            ...
            bool state = received >= 1 && _eq_recv0[ctx.Si] == _eq_recv1[ctx.Si];
            string output = state ? _eq_output[ctx.Si] : _eq_falseOutput[ctx.Si];
            if (ctx.OutRegs[0] >= 0)
                ctx.Regs[ctx.OutRegs[0]] = string.IsNullOrEmpty(output) ? null : output;
        }

        private static void EvalGreater(in SignalEvalContext ctx)
        {
            string in0 = ctx.InRegs[0] >= 0 ? ctx.Regs[ctx.InRegs[0]] : null;
            string in1 = ctx.InRegs[1] >= 0 ? ctx.Regs[ctx.InRegs[1]] : null;
            ...
            if (in0 != null)
            {
                float.TryParse(in0, NumberStyles.Any, CultureInfo.InvariantCulture, out _gt_val0[ctx.Si]);
                _gt_tsr0[ctx.Si] = 0f;
            }
            else _gt_tsr0[ctx.Si] += ctx.Dt;

            if (in1 != null)
            {
                float.TryParse(in1, NumberStyles.Any, CultureInfo.InvariantCulture, out _gt_val1[ctx.Si]);
                _gt_tsr1[ctx.Si] = 0f;
            }
            else _gt_tsr1[ctx.Si] += ctx.Dt;

            float tf = _gt_timeFrame[ctx.Si];
            int received = (_gt_tsr0[ctx.Si] <= tf ? 1 : 0) + (_gt_tsr1[ctx.Si] <= tf ? 1 : 0);

            // Vanilla GreaterComponent: sendOutput = true if ANY input within timeFrame,
            // then compares stored float values. We match this: >= 1.
            bool state = received >= 1 && _gt_val0[ctx.Si] > _gt_val1[ctx.Si];
            string output = state ? _gt_output[ctx.Si] : _gt_falseOutput[ctx.Si];
            if (ctx.OutRegs[0] >= 0)
                ctx.Regs[ctx.OutRegs[0]] = string.IsNullOrEmpty(output) ? null : output;
        }

        private static void EvalArith(SignalNodeType type, in SignalEvalContext ctx)
        {
            string in0 = ctx.InRegs[0] >= 0 ? ctx.Regs[ctx.InRegs[0]] : null;
            string in1 = ctx.InRegs[1] >= 0 ? ctx.Regs[ctx.InRegs[1]] : null;
            ...
            if (in0 != null) { _arith_recv0[ctx.Si] = in0; _arith_tsr0[ctx.Si] = 0f; }
            else _arith_tsr0[ctx.Si] += ctx.Dt;

            if (in1 != null) { _arith_recv1[ctx.Si] = in1; _arith_tsr1[ctx.Si] = 0f; }
            else _arith_tsr1[ctx.Si] += ctx.Dt;

            float tf = _arith_timeFrame[ctx.Si];
            int received = (_arith_tsr0[ctx.Si] <= tf ? 1 : 0) + (_arith_tsr1[ctx.Si] <= tf ? 1 : 0);

            if (received < 2)
            {
                if (ctx.OutRegs[0] >= 0) ctx.Regs[ctx.OutRegs[0]] = null;
                return;
            }

            if (!float.TryParse(_arith_recv0[ctx.Si], NumberStyles.Any, CultureInfo.InvariantCulture, out float a)) a = 0f;
            if (!float.TryParse(_arith_recv1[ctx.Si], NumberStyles.Any, CultureInfo.InvariantCulture, out float b)) b = 0f;
            ...
            float cMin = _arith_clampMin[ctx.Si];
            float cMax = _arith_clampMax[ctx.Si];
            if (cMax > cMin)
                result = Math.Clamp(result, cMin, cMax);

            if (ctx.OutRegs[0] >= 0)
                ctx.Regs[ctx.OutRegs[0]] = result.ToString("G", CultureInfo.InvariantCulture);
        }
        ...
        private static void EvalMemory(int si, int[] inRegs, int[] outRegs, string[] regs)
        {
            ...
            if (inRegs.Length > 1 && inRegs[1] >= 0 && regs[inRegs[1]] != null)
                _mem_writable[si] = regs[inRegs[1]].Length == 1 && regs[inRegs[1]][0] == '1';
            if (inRegs.Length > 2 && inRegs[2] >= 0 && regs[inRegs[2]] != null)
                _mem_writable[si] = regs[inRegs[2]].Length == 1 && regs[inRegs[2]][0] == '1';
