...
namespace ItemOptimizerMod
{
    ...
    static class ServerOptimizer
    {
        ...
        /// <summary>Per-thread shadow HashSet for server-side dedup (O(1) Queue.Contains). Serial access assumed within the ServerPeer.Update prefix/postfix — runs on the server thread only.</summary>
        [ThreadStatic] private static HashSet<Entity> _containsShadow;
        /// <summary>Per-thread shadow Queue tracking the source queue for incremental rebuild. Serial access assumed within ServerPeer.Update — runs on the server thread only.</summary>
