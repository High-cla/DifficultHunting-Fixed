...
| 场景 | 应优先用 | 再考虑用 |
|------|---------|---------|
| 批量文本替换 | `git-sed` (通过技能调用) | 代理 |
| 文件查找 | `fd-find` | 代理 |
| 内容搜索+统计 | `ripgrep` (rg) | 代理 |
| 模糊过滤 | `fzf-filter` | 代理 |
| 跨文件逻辑验证 | 工具组合 | 代理 |
| **需要理解语义/做决策** | — | 代理 ✅ |

### 实际案例
- 82处路径前缀替换 → `git-sed` 一条命令 < 1秒，而非代理35秒
- DDH→DH批量替换 → `git-sed` < 1秒，而非代理29秒
- NPC物品差异分析 → `rg -A` 直接读取对比，而非整个代理会话

### 原理
简单机械重复 → CLI工具。复杂逻辑/需要理解语义 → 代理。
